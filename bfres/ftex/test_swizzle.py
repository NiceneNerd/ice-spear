...
# result is the data after swizzling
result = swizzle(width, height, format_, tileMode, swizzle, pitch, data, do_swizzle)


def swizzle(width, height, format_, tileMode, swizzle_, pitch, data, toGFD=False):
    result_ = bytearray(data)
    ...
    for y in range(height):
        for x in range(width):
            ...
            if tileMode == 0 or tileMode == 1:
                ...
                pos = AddrLib_computeSurfaceAddrFromCoordMacroTiled(x, y, bpp, pitch, height, tileMode, pipeSwizzle,
                                                                    bankSwizzle)
