...
THREE.ShaderLib[ 'ocean_subtransform' ] = {
	...
	fragmentShader: [
		...
		'void main (void) {',
			...
			'fragmentColor = vec4(outputA, outputB);',
		'}'
	].join( '\n' )
};
	...
	fragmentShader: [
		...
		'void main (void) {',
			...
			'if (K.x == 0.0 && K.y == 0.0) {',
				'h = 0.0;', //no DC term
			'}',
			'fragmentColor = vec4(h, 0.0, 0.0, 0.0);',
		'}'
	].join( '\n' )
};
THREE.ShaderLib[ 'ocean_phase' ] = {
	...
	fragmentShader: [
		...
		'void main (void) {',
			...
			'fragmentColor = vec4(phase, 0.0, 0.0, 0.0);',
		'}'
	].join( '\n' )
};
THREE.ShaderLib[ 'ocean_spectrum' ] = {
	...
	fragmentShader: [
		...
		'void main (void) {',
			...
			'fragmentColor = vec4(hX + multiplyByI(h), hZ);',
		'}'
	].join( '\n' )
};
THREE.ShaderLib[ 'ocean_normals' ] = {
	...
	fragmentShader: [
		...
		'void main (void) {',
			...
			'fragmentColor = vec4(normalize(topRight + topLeft + bottomLeft + bottomRight), 1.0);',
		'}'
	].join( '\n' )
};
THREE.ShaderLib[ 'ocean_main' ] = {
	...
	vertexShader: [
		...
		'void main (void) {',
			'vec3 newPos = position + texture(u_displacementMap, uv).rgb * (u_geometrySize / u_size);',
			'vPos = newPos;',
			'vUV = uv;',
	...
	fragmentShader: [
		...
		'void main (void) {',
			'vec3 normal = texture(u_normalMap, vUV).rgb;',
			...
			'fragmentColor = vec4(hdr(color, u_exposure), 1.0);',
		'}'
	].join( '\n' )
};
