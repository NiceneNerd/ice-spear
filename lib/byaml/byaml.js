...
module.exports = class BYAML
{
    ...
    static toRawValues(obj)
    {
        if(obj instanceof BYAML_Value)
            return obj.value;

        for(let i in obj)
        {
            obj[i] = this.toRawValues(obj[i]);
        }
        return obj;
    }
    ...
    parse(pathOrBuffer)
    {
        this.parser = new Binary_File.Parser(pathOrBuffer);
    ...
    _parseNodeEntryValues(nodeEntry)
    {
        ...
        if(nodeType.type == "object")
        {
            ...
            {
                nodeEntry.value = new BYAML_Value(nodeEntry.type, this.getString(this.file.read("u32")));
            }else{
                nodeEntry.value = new BYAML_Value(nodeEntry.type, this.file.read(nodeType.dataType));
            }
        }
        return nodeEntry;
    }
