...
module.exports = class String_Table {
    ...
    _loadFromCache() {
        //return false;
        if (
            fs.existsSync(this.cachePathString) &&
            fs.existsSync(this.cachePathHash)
        ) {
    ...
    _saveToCache() {
        fs.writeFile(
            this.cachePathString,
            JSON.stringify(this.tableString, null, 4),
            () => {
                console.log(
                    "String Table cache saved. (%s)",
                    this.cachePathString
                );
            }
        );
        ...
        fs.writeFile(
            this.cachePathHash,
            JSON.stringify(this.tableHash, null, 4),
            () => {
                console.log(
                    "String Table cache saved. (%s)",
                    this.cachePathHash
    ...
    async load() {
        ...
        let rpxFile =
            this.config.getValue("game.basePath") + "/" + this.EXEC_PATH;
        let rpx = new RPX();
        rpx.parse(rpxFile);
        console.timeEnd("RPX");
        ...
        if (this.loader)
            await this.loader.setInfo(
                `Scanning Strings (this may take a minute)`
            );
        ...
        for (let textBuff of strTables) {
            ...
            while ((offsetEnd = textBuff.indexOf(0, offsetStart)) >= 0) {
                if (offsetEnd - offsetStart > 0) {
                    let str = this.textDecoder.decode(
                        textBuff.slice(offsetStart, offsetEnd)
                    );
                ...
                if (newTimestamp - lastTimestamp > 2500) {
                    ...
                    let perc = (
                        (offsetStart / textBuff.length) *
                        100
                    ).toPrecision(5);
                    if (this.loader)
                        await this.loader.setInfo(
                            `Scanning Strings (${i + 1}/${
                                strTables.length
                            }) ${perc}%`
                        );
                }
