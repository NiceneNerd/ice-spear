...
module.exports = class RPX {
     ...
     */
    parse(elfFile) {
        this.parser = new Binary_File.Parser(elfFile);
            ...
            if (section.offset != 0) {
                ...
                */
                ...
                if ((section.type & this.SHT_NOBITS) != this.SHT_NOBITS) {
                    ...
                    section.bufferRaw = this.file.buffer.slice(
                        dataOffset,
                        dataOffset + section.size
                    );

                    if (!section.compressed) section.buffer = section.bufferRaw;
                }
            }
        }
    }
     ...
     */
    getSectionData(type = null) {
        let sectionData = [];

        for (let section of this.header.sections) {
            if (
                section.offset != 0 &&
                (type === null || section.type == type)
            ) {
                this._decompressSection(section);

                if (section.buffer != null) sectionData.push(section.buffer);
            }
        }
