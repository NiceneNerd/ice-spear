...
module.exports = class Actor_Object_Loader
{
    ...
    async getObject(name)
    {
        ...
        if(!actorObject)
        {
            actorObject = await this._getActorObject("DUMMY_BOX", name); //@TODO put the box in the special-actor handler
        }
    ...
    async _getActorObject(name, aliasName = undefined)
    {
        if(!this.objects[name])
        {
            ...
            {
                objGroup.add(this.mubinRenderer.renderer.createInstancedModel(subModel));
            }
