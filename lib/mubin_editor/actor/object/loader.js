...
const Main_Config        = require("../../../../lib/config/main_config");
...
{
    constructor(actorPath, modelPath, texturePath, project, actorCache, mubinRenderer, loader, stringTable = null)
    {
    ...
    async init()
    {
        this.loadTextureFiles = this.config.getValue("fieldEditor.loadTextures");
        console.log("Load Textures? = " + this.loadTextureFiles);

        await this.loader.setStatus("Loading Actor Database");
        await this.loader.setInfo("Parsing File");

        let byaml = new BYAML.Parser();
        let actorInfoPath = path.join(this.project.getPath(), "content/Actor/ActorInfo.product.sbyml");
        if (!fs.existsSync(actorInfoPath))
            actorInfoPath = this.actorPath + "/ActorInfo.product.sbyml";
        this.actorData = byaml.parse(this.fileLoader.buffer(actorInfoPath));
...
*/
    ...
    async _getActorBfresFiles(bfresName, actorName)
    {
        ...
        if(this.bfresFiles[bfresMainPath] || fs.existsSync(bfresMainPath))
        {
            ...
            const baseBfresPath = path.join(this.texturePath, `content/Model/${bfresName}.sbfres`);
            if (fs.existsSync(baseBfresPath)) {
                files.add(await this._getSingleActorBfresFile(projectBfresPath));
                return files;
            }
            const projectBfresPath = path.join(this.project.getPath(), `content/Model/${bfresName}.sbfres`);
            if (fs.existsSync(projectBfresPath)) {
                files.add(await this._getSingleActorBfresFile(projectBfresPath));
                return files;
            }
            const infoBfresNum = parseInt(actorName.slice(-2));
            if (infoBfresNum !== NaN) {
                let bfresSubPath = path.join(this.modelPath, `${bfresName}-${infoBfresNum}.sbfres`);
                if (this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) {
                    files.add(await this._getSingleActorBfresFile(bfresSubPath));
                    return files;
                }
                bfresSubPath = path.join(this.texturePath, `${bfresName}-${infoBfresNum}.sbfres`);
                if (this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) {
                    files.add(await this._getSingleActorBfresFile(bfresSubPath));
                    return files;
                }
                bfresSubPath = path.join(this.project.getPath(), `${bfresName}-${infoBfresNum}.sbfres`);
                if (this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) {
                    files.add(await this._getSingleActorBfresFile(bfresSubPath))
                    return files;
                }
            ...
            for(let i=0;;++i)
            {
                ...
                let bfresSubPath = path.join(this.modelPath, `${bfresName}-${bfresNum}.sbfres`);
                if(this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) 
                {
                    files.add(await this._getSingleActorBfresFile(bfresSubPath));
                }else{
                    bfresSubPath = path.join(this.texturePath, `${bfresName}-${bfresNum}.sbfres`);
                    if(this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) 
                    {
                        files.add(await this._getSingleActorBfresFile(bfresSubPath));
                    }else{
                        bfresSubPath = path.join(this.project.getPath(), `${bfresName}-${bfresNum}.sbfres`);
                        if(this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) 
                        {
                            files.add(await this._getSingleActorBfresFile(bfresSubPath));
                        }
    ...
    async _getSingleActorBfresFile(bfresPath)
    {
        if(!this.bfresFiles[bfresPath])
        {
            ...
            const texturePath = path.join(this.texturePath, path.basename(bfresPathNoExt) + "Tex1.sbfres"); // @TODO also handle ".1.bfres" files....

            if(this.loadTextureFiles && fs.existsSync(texturePath))
            {
