...
module.exports = class Actor_Object_Loader
{
    ...
    async _getActorObject(name, aliasName = undefined)
    {
        if(!this.objects[name])
        {
            ...
            if(name == "TwnObj_TempleOfTime_A_01") {
                mainModel = undefined;
            }
...
*/
    ...
    async _getActorBfresFiles(bfresName, actorName)
    {
        ...
        let bfresMainPath = path.join(this.modelPath, `${bfresName}.sbfres`);

        if(actorName == "TwnObj_TempleOfTime_A_01")
        {
            bfresMainPath = "/home/mbeboek/graphicPacks/BreathOfTheWild_EventTest/content/Model/TwnObj_TempleOfTime_A-04.sbfres";
        }
