            ...
            if(!mainModel)
            {
                ...
                if(this.actorCache) {
                    try {
                        ...
                        console.error("Failed to load model for " + name);
...
*/
    ...
    async _getActorBfresFiles(bfresName, actorName)
    {
        ...
        if(this.bfresFiles[projectBfresPath] || fs.existsSync(projectBfresPath))
        {
            files.add(await this._getSingleActorBfresFile(projectBfresPath));
        }else{
            const baseBfresPath = path.join(this.texturePath, `${bfresName}.sbfres`);
            if (fs.existsSync(baseBfresPath)) {
                files.add(await this._getSingleActorBfresFile(baseBfresPath));
                return files;
            }
            ...
            for(let i=0;;++i)
            {
                ...
                let bfresSubPath = path.join(this.project.getPath(), `content/Model/${bfresName}-${bfresNum}.sbfres`);
                if(this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) 
                {
                    ...
                    if(this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) 
                    {
                        ...
                        bfresSubPath = path.join(this.modelPath, `${bfresName}-${bfresNum}.sbfres`);
                        if(this.bfresFiles[bfresSubPath] || fs.existsSync(bfresSubPath)) 
                        {
                            files.add(await this._getSingleActorBfresFile(bfresSubPath));
                        } else {
