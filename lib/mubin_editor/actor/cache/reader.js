...
module.exports = class Actor_Cache_Reader
{
    ...
    async read(actorName)
    {
        try{
            ...
            for(const jsonData of Object.values(jsonDataArray))
            {
                ...
                if(jsonData.textureEmission)
                    jsonData.textureEmission = this._readTexture(jsonData.textureEmission);
            }
            return jsonDataArray;
            
        }catch(e){
            //console.warn(e);
            return undefined; // this is totally fine, it just means there is no cache
        }
    }
    ...
    _readTexture(info)
    {
        ...
        for(let i=3; i<info.imageBuffer.byteLength; i+=4)
        {
            if(info.imageBuffer[i] < 200)
            {
                ++stats.countAlpha;
                stats.sizeAlpha += info.imageBuffer.byteLength;
                console.log(stats);
                return info;
            }
        }
        ...
        ++stats.countColor;
        stats.sizeColor += info.imageBuffer.byteLength;
        console.log(stats);
        return info;
    }
