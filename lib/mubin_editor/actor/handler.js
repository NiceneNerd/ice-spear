...
module.exports = class Actor_Handler
{
     ...
     */
    constructor(shrineRenderer, cachePath, loader, stringTable = null)
    {
        ...
        this.actorCache = new Actor_Cache(cachePath);
    ...
    {
        //if(name != "Animal_Kokko_A" && name != "DUMMY_BOX")return undefined;

        if(!this.objects[name])
        {
            //console.time("ReadCache-" + name);
            let mainModel = await this.actorCache.get(name);
            //console.timeEnd("ReadCache-" + name);
