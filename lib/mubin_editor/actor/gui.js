...
module.exports = class Actor_GUI
{
    ...
    _addCallbacks()
    {
        ...
        this.nodeCache.get(".data-tool-deselect").onclick = () => this.actor.handler.deselectActor(this.actor);
        this.nodeCache.get(".data-tool-focus").onclick = () => this.actor.handler.focusActor(this.actor);

        this.nodeCache.get(".data-tool-copy").onclick = () => this.actor.copy();
        this.nodeCache.get(".data-tool-delete").onclick = () => this.actor.delete();

        this.nodeCache.get(".data-tool-addRotScalar").onclick = () => this.actor.addRotationParams(false);
        this.nodeCache.get(".data-tool-addRotVector").onclick = () => this.actor.addRotationParams(true);

        this.nodeCache.get(".data-tool-addScaleScalar").onclick = () => this.actor.addScalingParams(false);
        this.nodeCache.get(".data-tool-addScaleVector").onclick = () => this.actor.addScalingParams(true);

        this.nodeCache.get(".data-tool-removeRot").onclick = () => this.actor.removeRotationParams();
        this.nodeCache.get(".data-tool-removeScale").onclick = () => this.actor.removeScalingParams();

        this.nodeCache.get(".data-tool-copyJSON").onclick = () =>
        {
            const json = this.actor.getParamJSON();
            clipboard.writeText(json);
        };
        
        this.nodeCache.get(".data-tool-importJSON").onclick = () => this._importJSON(clipboard.readText(), true);
        ...
        this.nodeCache.get(".data-tool-byamlToActor").onclick = () => this._editorJsonToActor();
        this.nodeCache.get(".data-tool-byamlRefresh").onclick = () => this.update();
    }
