...
module.exports = class Mubin_Renderer {
    ...
    constructor(canvasNode, uiNode, loader) {
        ...
        this.selectedActorList = this.uiNode.querySelector(
            ".container-selectedActors"
        );
    ...
    init(actorFinder) {
        this.actorSearchGui = new Actor_Search_GUI(
            this.uiNode.querySelector(".window-container"),
            actorFinder
        );
    }
    ...
    selectActor(actor) {
        ...
        const addedChild =
            this.selectedActorList.children[
                this.selectedActorList.children.length - 1
            ]; // only way to get a correct reference
        actor.gui = new Actor_GUI(actor, addedChild, this.history, this.loader);
        actor.gui.update();
    }

    deselectActor(actor) {
        if (actor.gui) this.selectedActorList.removeChild(actor.gui.node);
    }
