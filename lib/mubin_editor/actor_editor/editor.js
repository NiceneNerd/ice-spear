        ...
        document.addEventListener("keydown", (ev) => {
            if (!ev.ctrlKey && !ev.shiftKey && !ev.altKey) {
                if (ev.key == "x") {
                    ...
                    if (this.lockX)
                        document.querySelector(".data-footer").innerHTML += "<div id='xlock'>X Locked</div>";
                    else
                        document.querySelector("#xlock").remove();
                }
                if (ev.key == "z") {
                    this.lockZ = !this.lockZ;
                    if (this.lockZ)
                        document.querySelector(".data-footer").innerHTML += "<span id='zlock'>Z Locked</span>";
                    else
                        document.querySelector("#zlock").remove();
                }
            }
        });
    }
    ...
    eventActorMove(ev, camera)
    {
        ...
        if(ev.ctrlKey) // scaling
        {
            ...
            if(ev.type == "wheel")
            {
                ...
                console.log(this.lockX, this.lockZ);
                const moveVec = new THREE.Vector2(ev.movementX, -ev.movementY);            
