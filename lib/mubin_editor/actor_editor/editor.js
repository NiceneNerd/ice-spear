...
module.exports = class Actor_Editor {
    ...
    constructor(mubinRenderer, actorHandler) {
        ...
        this.actorRaytracer = new Actor_Raytracer((...data) =>
            this.eventActorSelected(...data)
        );

        this.eventEmitter =
            this.mubinRenderer.renderer.getSelectorEventEmitter();
        this.eventEmitter.on("move", (...data) => this.eventActorMove(...data));
        this.eventEmitter.on("end", () => this.eventMoveEnd());
        this.eventEmitter.on("select", (...data) =>
            this.actorRaytracer.raytrace(...data)
        );
                ...
                if (ev.key == "x") {
                    ...
                    if (this.lockX)
                        document.querySelector("#footer").innerHTML +=
                            "<div id='xlock'>X Axis Locked</div>";
                    else document.querySelector("#xlock").remove();
                }
                if (ev.key == "z") {
                    this.lockZ = !this.lockZ;
                    if (this.lockZ)
                        document.querySelector("#footer").innerHTML +=
                            "<div id='zlock'>Z Axis Locked</div>";
                    else document.querySelector("#zlock").remove();
                }
            }
        });
    }
    ...
    eventActorSelected(actorInstances, isMouseUp, mouseMoved) {
        if (actorInstances.length == 0) {
            if (!mouseMoved) this._resetSelection();
        } else {
            ...
            if (actor) {
                if (!isMouseUp) {
                    ...
                    !mouseMoved &&
                    !this.actorAdded &&
                    this.selectedActors.includes(actor)
                ) {
    ...
    eventActorMove(ev, camera) {
        ...
        if (ev.ctrlKey) {
            ...
            if (ev.type == "wheel") {
                speedVec = {
                    y: 1.0 + (ev.deltaY < 0 ? speedScale : -speedScale)
                };
            } else {
            ...
            if (ev.type == "wheel") {
                ...
                const moveVec = new THREE.Vector2(ev.movementX, -ev.movementY);
                moveVec.rotateAround(
                    new THREE.Vector2(0.0, 0.0),
                    camera.rotation.y % (Math.PI * 2)
                );

                speedVec = {
                    x: this.lockX ? 0.0 : moveVec.x * speedMulti,
                    z: this.lockZ ? 0.0 : -moveVec.y * speedMulti
                };
            }
        }
    ...
    selectActor(actor) {
        if (this.selectedActors.includes(actor)) return;
