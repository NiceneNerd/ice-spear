...
module.exports = class BFRES_Parser {
    ...
    getFileParser(type, index) {
        if (
            ...
            this.files[type].entries[index] != null &&
            this.files[type].entries[index].parser != null
        ) {
    ...
    async parseFileTable() {
        for (let type = 0; type < this.header.fileOffsets.length; ++type) {
            ...
            if (tablePos == 0 || this.header.fileCounts[type] == 0) continue;

            this.parser.pos(tablePos);
            this.files[type] = this.parser.parse(
                require("./index_entries.json")
            );
            ...
            for (let entry of this.files[type].entries) {
                ...
                if (
                    this.autoLoad &&
                    fileInfo.preload === true &&
                    entry.namePointer != 0
                ) {
                    const Parser_Class = requireGlobal(fileInfo.parser);
                    entry.parser = new Parser_Class(
                        this.parser,
                        entry,
                        this.contentType
                    );
    ...
    detectContentType() {
        ...
        if (name.endsWith("Tex1")) this.contentType = Content_Types.TEXTURE;
        else if (name.endsWith("Tex2")) this.contentType = Content_Types.MIPMAP;
    }
