...
const FTEX_Swizzler      = require('./swizzler');
...
module.exports = class FTEX_Parser
{
    ...
    _generateColorBuffer(img, texTypeInfo, rawBuffer)
    {
        ...
        {
            // @TODO port this to C++, it's slow as hell...

            console.log("Size: %d %d %d", img.sizeX, img.sizeY, img.sizeZ);
            console.time("Swizzle");
            let swizzler = new FTEX_Swizzler();
            let deSwizzledBuffer = swizzler.deswizzle(img.sizeX, img.sizeY, img.sizeZ, index, img.textureFormat, img.tileMode, img.swizzleValue, img.pitch, texTypeInfo.bpBlock, rawBuffer);
            ...
            bufferArray[index] = blockHandler.decode(index);
        }
        return bufferArray;
    }
};
