    ...
    parseMaterials() {
        for (let index of this.header.fmatIndex) {
            for (const model of Object.values(this.models)) {
                ...
                if (!entry || entry.namePointer == 0) continue;
                ...
                for (let texRef of fmatData.textureRef) {
                    if (texRef.headerOffset == 0 && this.textureParser != null)
                        texRef.texture = this.textureParser.getTextureByName(
                            texRef.name
                        );
                    else console.warn("FMAT: model has own texture!");

                    if (texRef.texture == null) {
                        continue;
                    }

                    let prefix = texRef.name.endsWith("_norm")
                        ? "Nrm"
                        : texRef.name.substr(-3);
                    switch (prefix) {
                        default:
                        case "Alb":
                            if (!model.textureColor)
                                // @TODO check hoe to better handle this
                ...
                for (let renderInfoEntry of fmatData.renderParamsIndex
                    .entries) {
                    if (renderInfoEntry.namePointer != 0) {
                        this.parser.pos(renderInfoEntry.dataPointer);
                        let renderInfo = this.parser.parse(
                            require("./render_info_params.json")
                        );
                        ...
                        for (let i = 0; i < renderInfo.arrayCount; ++i) {
                            switch (renderInfo.type) {
                                ...
                                case 2: // string
                                    let strOffset =
                                        file.pos() + file.read("u32");
                                    let strLen = file.read(
                                        "u32",
                                        strOffset - 4
                                    );
                                    ...
                                    renderInfo.data.push(
                                        file.readString(strLen, strOffset)
                                    );
                                    break;
                                default:
                                    console.log(
                                        "FMAT: unknown render info array type: '%d'",
                                        renderInfo.type
                                    );
                                    break;
                            }
                        ...
                        fmatData.matRenderInfo[renderInfo.name] =
                            renderInfo.data;
                    }
                }
            ...
            for (let entry of index.entries) {
                if (!isFirst) {
                    ...
                    if (fshpData.lodModel.length > 0) {
                        ...
                        let indexCount =
                            lodModel.indexBuffer.buffer.length / format.size;
                        let indexIndex = 0;
                        let indexArray =
                            format.size == 2
                        ...
                        let primitypeInfo =
                            this.primitypeTypes[lodModel.primitypeType];
                        let bufferReader = new Binary_File(
                            lodModel.indexBuffer.buffer
                        );
                        ...
                        this.models[modelNumber].bfresName =
                            this.header.fileName;
                        this.models[modelNumber].materialIndex =
                            fshpData.fmatIndex;
                    }
                }
    ...
    parseVertexData() {
        for (let fvtxData of this.header.fvtxData) {
            for (let attr of fvtxData.attributes) {
                ...
                if (bufferType.bufferName == null) continue;
                ...
                model[nameIndex] = 0;
                model[nameArray] = new Float32Array(
                    fvtxData.vertexCount * bufferType.size
                );
                ...
                for (
                    ...
                    for (let i = 0; i < formatInfo.count; ++i) {
                        ...
                        if (i < bufferType.size)
                            model[nameArray][model[nameIndex]++] =
                                this.convertType(
                                    rawVal,
                                    formatInfo.typeIn,
                                    formatInfo.typeOut
                    ...
                    this.parser.pos(
                        this.parser.file.pos() + bufferHeader.stride - sizeRead
                    );
                }
            }
        }
