...
module.exports = class Filter {
    ...
    _initEvents() {
        ...
        this.inputSearch.onkeydown = function (ev) {
            if (ev.key == "Enter") that._triggerFilteredElement();
        };
    }

    _triggerFilteredElement() {
        let items = this.nodeSearch.querySelectorAll(
            this.rowSelector + ":not([hidden])"
        );
        if (items.length > 0) {
            items[0].click();
        }
        ...
        for (let item of items) {
            ...
            if (this.valueSelector == null) {
                ...
                if (
                    isRegex
                        ? text.test(htmlText)
                        : htmlText.toLowerCase().includes(text)
                )
                    item.hidden = false;
            } else {
