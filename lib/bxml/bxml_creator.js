...
module.exports = class BXML_Creator {
            ...
            if (dataTypeInfo != null && node.hasValue()) {
                ...
                if (dataType == "string") {
                    ...
                    if (!this.stringTable.has(str)) {
                        this.stringTable.set(
                            str,
                            this.headerData.stringBufferSize
                        );

                        this.headerData.stringBufferSize +=
                            Buffer.byteLength(str, "utf8") + 1; // +1 = ending zero
                        this.headerData.stringBufferSize =
                            Math.ceil(
                                this.headerData.stringBufferSize /
                                    STRING_PADDING
                    ...
                    let rawValues = this.fileCreator.file.convert(
                        dataType,
                        node.val()
                    );
    ...
    _calcOffsets() {
        this.offsetDataTable =
            SIZE_HEADER +
            SIZE_ROOT_NODE * this.headerData.rootNodesCount +
            SIZE_NODE *
                (this.headerData.nodesCount +
                    this.headerData.rootChildNodesCount);

        this.offsetStringTable =
            this.offsetDataTable + this.headerData.dataBufferSize;
    }
    ...
    create(bxmlNode = null) {
        if (bxmlNode == null) bxmlNode = this.header.node;
        ...
        this.headerData.nodesCount =
            bxmlNode.getChildCount(true) - this.headerData.rootChildNodesCount;
        ...
        let buffer = this.fileCreator.write(
