	...
	getPrepareMaskMaterial: function () {
		...
		return new THREE.ShaderMaterial( {
			...
			fragmentShader:
				...
				\
				void main() {\
					float depth = unpackRGBAToDepth(textureProj( depthTexture, projTexCoord ));\
					float viewZ = -perspectiveDepthToViewZ( depth, cameraNearFar.x, cameraNearFar.y );\
					float depthTest = (-vPosition.z > viewZ) ? 1.0 : 0.0;\
					fragmentColor = vec4(0.0, depthTest, 1.0, 1.0);\
				}"
		} );
		...
		return new THREE.ShaderMaterial( {
			...
			fragmentShader:
				...
				void main() {\n\
					...
					fragmentColor = vec4(edgeColor, 1.0) * vec4(d);\
				}"
		} );
		...
		return new THREE.ShaderMaterial( {
			...
			fragmentShader:
				...
				void main() {\
					...
					fragmentColor = vec4(diffuseSum/weightSum, 1.0);\
				}"
		} );
