...
module.exports = class Renderer
{
    ...
    addModel(model)
    {
        ...
        let map = null;
        if(model.textureColor != null)
        {
            let tex = model.textureColor;
            map = new THREE.DataTexture(tex.imageBuffer, tex.sizeX, tex.sizeY, THREE.RGBAFormat );
            map.needsUpdate = true;
        }else{
            map = new THREE.TextureLoader().load("assets/img/texture_dummy.png");
        }
        //let map = new THREE.TextureLoader().load("assets/img/base.png");

        //let material = new THREE.MeshBasicMaterial( { vertexColors: THREE.VertexColors, map } );
