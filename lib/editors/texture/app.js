...
module.exports = class Editor_Texture {
    constructor(node, bfresParser, entry) {
        ...
        this.embededParser = mainApp.bfresParser.getFileParser(
            BFRES_FileTypes.types.EMBEDDED,
            1
        );
    ...
    loadData() {
        ...
        if (this.entry.parser.header.surface != null) this.renderData();
    ...
    exportImage(format, index) {
        ...
        const imgBuffer = ctx.getImageData(
            0,
            0,
            ftex.surface.sizeX,
            ftex.surface.sizeY
    ...
    renderData(fileOffset) {
        ...
        this.node.querySelector(".data-header-filePath").innerHTML =
            ftex.filePath;
        this.node.querySelector(".data-header-sizeX").innerHTML =
            ftex.surface.sizeX;
        this.node.querySelector(".data-header-sizeY").innerHTML =
            ftex.surface.sizeY;
        this.node.querySelector(".data-header-sizeZ").innerHTML =
            ftex.surface.sizeZ;

        this.node.querySelector(".data-header-mipmapCount").innerHTML =
            ftex.surface.mipmapCount;
        this.node.querySelector(".data-header-textureFormat").innerHTML =
            this.entry.parser.textureTypes[ftex.surface.textureFormat].name;
        this.node.querySelector(".data-header-colorChannels").innerHTML =
            ftex.surface.colorChannels;

        if (this.embededParser != null) {
            let buff = this.embededParser.getTextureInfo(
                this.entry.fileIndex - 1
            );

            this.node.querySelector(".data-header-embededInfo").innerHTML = buff
                .toString("hex")
                .toUpperCase()
                .match(/[0-9A-Z]{2}/g)
                .join(" ");
            this.node.querySelector(".data-header-embededInfo").innerHTML +=
                "<br/>";
        ...
        this.indexSelect.onchange = () =>
            this.renderImage(this.indexSelect.value);
    }
    ...
    renderImage(index = 0) {
        ...
        if (img.colorChannels == 4) {
            ...
            for (let p = 0; p < pixels; ++p) {
                for (let c = 0; c < 4; ++c) {
                    idata.data[imgPos++] =
                        c >= img.colorChannels ? 255 : imageBuffer[bufferPos++];
                }
            }
        }
