...
module.exports = class Editor_Texture
{
    ...
    exportImage(format)
    {
        ...
        if(format == "png")
        {
            let pngBuffer = PNG.encode(img.imageBuffer, ftex.surface.sizeX, ftex.surface.sizeY);
    ...
    renderImage()
    {
        ...
        canvas.height = img.sizeY * 10; //img.imageBuffers.length;

        let ctx = canvas.getContext('2d');

        let offsetY = 0;
        for(let imageBuffer of img.imageBuffers)
        {
            let idata = ctx.createImageData(img.sizeX, img.sizeY);
            idata.data.set(imageBuffer);
            ctx.putImageData(idata, 0, offsetY);
            offsetY += img.sizeY * 1;
        }
    }
};
