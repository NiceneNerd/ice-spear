...
module.exports = class Editor_Model {
    constructor(node, bfresParser, entry) {
        ...
        this.threeJsRenderer = new Renderer(
            this.node.querySelector(".canvasWindow canvas")
        );
        ...
        Split(
            [
                ...
                this.node.querySelector(".sidebar-3")
            ],
            {
                sizes: [20, 70, 10],
                minSize: 0,
        ...
        this.filterMaterial = new Filter(
            this.node.querySelector("input.search-material"),
            this.node.querySelector(".data-fmat-names")
        );

        this.cameryTypeNode = this.node.querySelector(
            "select.data-renderSettings-cameraType"
        );
        this.cameryTypeNode.onchange = ev => {
            this.threeJsRenderer.changeCameraType(ev.target.value);
        };
    ...
    exportOBJ() {
        ...
        if (path != null && Object.keys(models).length > 0) {
    ...
    renderData() {
        ...
        this.node.querySelector(".data-header-fileName").innerHTML =
            this.entry.parser.header.fileName;
        this.node.querySelector(".data-header-filePath").innerHTML =
            this.entry.parser.header.filePath;

        this.node.querySelector(".data-header-fvtxCount").innerHTML =
            this.entry.parser.header.fvtxCount;
        this.node.querySelector(".data-header-fshpCount").innerHTML =
            this.entry.parser.header.fshpCount;
        this.node.querySelector(".data-header-fmatCount").innerHTML =
            this.entry.parser.header.fmatCount;
        this.node.querySelector(".data-header-userDataCount").innerHTML =
            this.entry.parser.header.userDataCount;

        this.threeJsRenderer.start();

        for (let i in this.entry.parser.models) {
            const subModel = this.threeJsRenderer.createModel(
                this.entry.parser.models[i]
            );
        ...
        for (let mat of this.entry.parser.materials) {
            ...
            for (let texRef of mat.textureRef) {
                names.push(
                    texRef.name + (texRef.headerOffset == 0 ? " (*)" : "")
                );
            }
            ...
            fmatNameHtml += `<tr data-modelName="${
                mat.name
            }" data-active="1" class="active data-row">
        ...
        for (let tr of trArray) {
            tr.ondblclick = function () {
                ...
                that.threeJsRenderer.scene.getObjectByName(modelName).visible =
                    isActive;
        ...
        let tableRenderInfo = fmatNameTbody.querySelectorAll(
            "table.renderInfo-table"
        );
