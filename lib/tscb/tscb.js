...
module.exports = class TSCB {
    ...
    static getSectionTilesByPos(
        tiles,
        lodScale,
        sectionMidpoint,
        sectionWidth
    ) {
        const sectionWidthHalf = 0.5 + 0.5; // add an bit of size to get a edge around the section
        ...
        for (let tile of tiles) {
            if (tile.lodScale == lodScale) {
                ...
                if (
                    tPos[0] - lodScaleHalf <= sPos[0] + sectionWidthHalf && // X
    ...
    static getTileGridRect(tiles, lodScale) {
        ...
        for (let tile of tiles) {
            if (tile.lodScale == lodScale) {
                if (tile.center[0] - lodScaleHalf < tileGridRect.min[0])
                    // Min - X
                    tileGridRect.min[0] = tile.center[0] - lodScaleHalf;

                if (tile.center[1] - lodScaleHalf < tileGridRect.min[1])
                    // Min - Y
                    tileGridRect.min[1] = tile.center[1] - lodScaleHalf;

                if (tile.center[0] + lodScaleHalf > tileGridRect.max[0])
                    // Max - X
                    tileGridRect.max[0] = tile.center[0] + lodScaleHalf;

                if (tile.center[1] + lodScaleHalf > tileGridRect.max[1])
                    // Max - Y
