...
module.exports = class Project_Manager {
    ...
    async create(name) {
        const projectPath = path.join(
            this.mainConfig.getValue("projects.path"),
    ...
    async open(name) {
        return await this.openPath(
            path.join(this.mainConfig.getValue("projects.path"), name)
        );
    }
    ...
    async _fixProjectStructure(projectPath) {
        ...
        for (const dirPath of PROJECT_DIRECTORIES) {
            projectDirPromises.push(
                fs.ensureDir(path.join(projectPath, dirPath))
            );
        }
    ...
    async openPath(projectPath) {
        if (!(await this.isValidProjectPath(projectPath))) return false;
    ...
    async isValidProjectPath(projectPath) {
        if (!(await fs.exists(projectPath))) return false;
