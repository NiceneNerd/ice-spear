...
module.exports = class Terrain_Mesh_Creator {
    ...
    async loadTerrainTexture() {
        this.textureArray = await loadTerrainTextures(
            this.gamePath,
            this.cachePath,
            this.gl
        );
    ...
    async createTileMesh(meshBuffer, waterBuffer, materialBuffer) {
        ...
        if (waterBuffer) {
            const vertexWaterBuffer = this._createWaterVertexBuffer(
                waterBuffer,
                1
            );
            staticWaterIndexBuffer =
                staticWaterIndexBuffer ||
                this._createIndexBuffer(
                    INDEX_COUNT_SIDE_WATER,
                    MAP_TILE_LENGTH_WATER
                );
            const waterShader = new THREE.MeshNormalMaterial({
                transparent: true,
                opacity: 0.3
            });
            meshArray.push(
    ...
    async _createSingleMesh(
        ...
        geometry.addAttribute(
            "materialMap",
            new THREE.BufferAttribute(texIndexBuffer, 3, false)
        );
    ...
    _createWaterVertexBuffer(meshBuffer) {
        ...
        for (let y = 0; y < MAP_TILE_LENGTH_WATER; ++y) {
            ...
            for (let x = 0; x < MAP_TILE_LENGTH_WATER; ++x) {
                ...
                const heightValue =
                    meshBuffer.readUInt16LE(bufferIndex) * MAP_HEIGHT_SCALE;
                h.push(heightValue);
                bufferIndex += 8;
    ...
    _createIndexBuffer(
        indexCountSide = INDEX_COUNT_SIDE,
        tileLength = MAP_TILE_LENGTH
    ) {
    ...
    _createTexIndexBuffer(materialBuffer) {
        ...
        for (let i = 0; i < materialBuffer.length; i += 4) {
            texIndexBuffer[texIndexBufferIndex++] =
                TEXTURE_INDEX_MAP[materialBuffer[i]]; // texture index 0
            texIndexBuffer[texIndexBufferIndex++] =
                TEXTURE_INDEX_MAP[materialBuffer[i + 1]]; // texture index 1
