...
module.exports = class Terrain
{
    ...
    async loadSectionMesh(sectionName, lodLevel = LOD_LEVEL_MAX)
    {
        ...
        for(const tile of sectionTiles)
        {
            ...
            const material = this._loadTileMaterial(tile);
            const mesh = this._loadTileMesh(tile, material);
            if(!mesh) {
    ...
    _loadTileMesh(tile, material)
    {
        const ssteraPackName = `${this._getTilePackName(tile.name)}.hght.sstera`;

        let sarc = this.loadedPacks[ssteraPackName] || this._loadTilePack(ssteraPackName);
        this.loadedPacks[ssteraPackName] = sarc;
        
        const mapBuffer = sarc.getFile(`${tile.name}.hght`);

        return createMeshFromBuffer(mapBuffer, material);
    }

    _loadTileMaterial(tile)
    {
        const ssteraPackName = `${this._getTilePackName(tile.name)}.mate.sstera`;
        ...
        const materialBuffer = new Float32Array(mapBuffer.length);
        for(let i=0; i<mapBuffer.length; ++i)
        {
            materialBuffer[i] = mapBuffer[i] / 255;
        }
