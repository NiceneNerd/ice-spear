...
module.exports = class Terrain {
    ...
    constructor(project, gl, loader) {
        ...
        this.gamePath = this.mainConfig.getValue("game.basePath");
        this.loader = loader;

        this.tscbData = undefined;
        this.pathMainField = path.join(
            this.gamePath,
            "content",
            "Terrain",
            "A",
        ...
        const cachePath = this.mainConfig.getValue("cache.terrainTextures")
            ? project.getCachePath()
            : "";
        this.meshCreator = new Terrain_Mesh_Creator(
            this.gamePath,
            cachePath,
            gl,
            this.loader
    ...
    async loadSectionMesh(sectionName, lodLevel = LOD_LEVEL_MAX) {
        ...
        const sectionTiles = TSCB.getSectionTilesByPos(
            this.tscbData.tiles,
            lodScale,
            sectionMidpoint,
            SECTION_WIDTH
        );
        const tileSectionScale =
            (TILE_GRID_SIZE_X / (LOD_MIN / lodScale)) *
            SECTION_WIDTH *
            TILE_TO_SECTION_SCALE;
        ...
        for (const tile of sectionTiles) {
            await this.loader.setInfo(
                `Tile ${tileNum++} / ${sectionTiles.length}`
            );
    ...
    async createTile(tile, tileSectionScale) {
        ...
        const meshArray = await this.meshCreator.createTileMesh(
            meshBuffer,
            waterBuffer,
            materialBuffer
        );
        ...
        for (const mesh of meshArray) {
            mesh.position.x =
                tile.center[0] * SECTION_WIDTH * TILE_TO_SECTION_SCALE;
            mesh.position.z =
                tile.center[1] * SECTION_WIDTH * TILE_TO_SECTION_SCALE;
