...
module.exports = class App extends App_Base {
    constructor(window, args) {
        ...
        this.fileFilter = new Filter(
            this.tabNodeTabs.querySelector(".list-group-header input"),
            this.tabNodeTabs,
            ".list-group-item"
        );
        ...
        for (let tab of tabs) {
            tab.onclick = () => {
                for (let otherTab of tabs) otherTab.classList.remove("active");
                ...
                for (let tabContent of this.tabNodeContent.children) {
                    tabContent.hidden =
                        tabContent.getAttribute("data-tab") != tabName;
                }
            };
        }
    ...
    initValues() {
        ...
        for (let node of this.valueNodes) {
            const nodeVal = this.config.getValue(
                node.getAttribute("data-configRef")
            );
            const nodeType = node.getAttribute("data-dataType");

            if (nodeType == "bool") node.checked = nodeVal;
            else node.value = nodeVal;
        }

        const cachePath = this.project.getCachePath();
        const cacheSizeNode = this.node.querySelector(
            ".data-info-projectCache"
        );
