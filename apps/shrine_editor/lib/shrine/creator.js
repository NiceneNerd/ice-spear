...
module.exports = class Shrine_Creator {
    ...
    getPackFilePath(shrineName) {
        return path.join(
            this.project.getShrinePath("build"),
            shrineName + ".pack"
        );
    }
    ...
    async save(shrineDir, shrineName, packData) {
        ...
        const backupName = `${shrineName}.${dateformat(
            new Date(),
            "yyyy-mm-dd_HH_MM_ss"
        )}.pack`;
        const backupPath = path.join(
            this.project.getShrinePath("backup"),
            backupName
        );
    ...
    async saveActors(typeName, shrineDir, shrineName) {
        const mapDir = shrineName.startsWith("Remains")
            ? "MainFieldDungeon"
            : "CDungeon";
        ...
        const actorPath = path.join(
            ...
            mapDir,
            shrineName,
            `${shrineName}_${typeName}.smubin`
        );
        const byaml = new BYAML.Creator();

        const byamlData = {
            ...(typeName == "Dynamic"
                ? this.actorHandler.dataActorDyn
                : this.actorHandler.dataActorStatic)
        };
        byamlData.Objs = byamlData.Objs.sort(
            (a, b) => a.HashId.value - b.HashId.value
        );

        this.actorHandler.dataActorDyn.Objs;
        const actorBuffer = byaml.create(
            typeName == "Dynamic"
                ? this.actorHandler.dataActorDyn
                : this.actorHandler.dataActorStatic
        );
