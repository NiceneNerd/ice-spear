...
module.exports = class Camera {
    constructor(engine) {
        ...
        this.cameraUniform = this.engine.createUniformBuffer([
            this.posBuffer,
            this.scaleBuffer
        ]);
    }
    ...
    update() {
        ...
        if (
            Math.abs(this.velocity[0]) > velocityMinVal ||
            Math.abs(this.velocity[1]) > velocityMinVal
        ) {
            ...
            this.posBuffer[0] = clamp(
                this.posBuffer[0],
                posLimit.x[0],
                posLimit.x[1]
            );
            this.posBuffer[1] = clamp(
                this.posBuffer[1],
                posLimit.y[0],
                posLimit.y[1]
            );
        ...
        if (Math.abs(this.scaleVelocity) > scaleVelocityMinVal) {
            this.scaleBuffer[0] +=
                this.scaleBuffer[0] * this.scaleVelocity * frameScalar;
            this.scaleBuffer[1] +=
                this.scaleBuffer[1] * this.scaleVelocity * frameScalar;

            this.scaleBuffer[0] = clamp(
                this.scaleBuffer[0],
                scaleLimitMin,
                scaleLimitMax
            );
            this.scaleBuffer[1] = clamp(
                this.scaleBuffer[1],
                scaleLimitMin,
                scaleLimitMax
            );
            ...
            this.cameraUniform
                .set(UNIFORM_INDEX_SCALE, this.scaleBuffer)
                .update();
