...
module.exports = class Icon {
    ...
    selectIcon(icon, val, update = true) {
        if (icon.index < 0 || icon.index >= this.selectionBuffer.length) return;
    ...
    updateSelectionBuffer(update = true) {
        if (update) this.selectionVertexBuffer.data(this.selectionBuffer);
    }

    async createDrawCall(locations) {
        this.iconShrineBuffer = await fs.readFile(
            path.join(__BASE_PATH, "assets", "img", "map", "shrine.bin")
        );
        ...
        for (let entry of locations) {
            if (
                entry.Icon &&
                (entry.Icon.value == "Dungeon" ||
                    entry.Icon.value.includes("Remains"))
            ) {
                const pos = {
        ...
        this.selectionVertexBuffer = this.glApp.createVertexBuffer(
            PicoGL.FLOAT,
            1,
            this.selectionBuffer
        );

        const instanceBuffer = this.glApp.createVertexBuffer(
            PicoGL.FLOAT,
            2,
            new Float32Array(instPosArray)
        );

        const iconVertexArray = this.engine.meshHelper
            .createQuad(this.radius)
            .instanceAttributeBuffer(2, instanceBuffer)
            .instanceAttributeBuffer(3, this.selectionVertexBuffer);
        const iconShrineTexture = this.engine.createTexture(
            this.iconShrineBuffer,
            { x: 128, y: 128 },
            {
        ...
        return this.engine
            .createDrawCall("icon", iconVertexArray, true)
