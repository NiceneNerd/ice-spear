...
module.exports = class World_Map {
    constructor(ui, basePath, updatePath, cachePath, loader) {
        ...
        this.textureHandler = new World_Map_Texture(
            texBasePath,
            this.cachePath,
            mapTilesX,
            mapTilesY,
    ...
    async load() {
        ...
        await this.engine.createShader(
            "map",
            path.join(this.shaderPath, "map.glsl.vert"),
            path.join(this.shaderPath, "map.glsl.frag")
        );
        await this.engine.createShader(
            "icon",
            path.join(this.shaderPath, "icon.glsl.vert"),
            path.join(this.shaderPath, "icon.glsl.frag")
        );
        await this.engine.createShader(
            "marker",
            path.join(this.shaderPath, "marker.glsl.vert"),
            path.join(this.shaderPath, "marker.glsl.frag")
        );

        const mapTexture = this.engine.createTexture(
            mapImageBuffer,
            this.textureHandler.getSize(),
            {
                format: PicoGL.RGB,
        ...
        this.mapDrawCall = this.engine
            .createDrawCall("map", mapVertexArray, true)
        ...
        (await this.marker.createDrawCall()).uniformBlock(
            "cameraUniforms",
            this.camera.getUniformBuffer()
        );
        (
            await this.icons.createDrawCall(this.locations.getLocations())
        ).uniformBlock("cameraUniforms", this.camera.getUniformBuffer());
